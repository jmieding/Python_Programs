...
def cake_heist_calculator(raw_cakes, capacity):
  ...
  cakes_with_ratio = [] # holds raw cakes with their $/lb ratio.
...
# add the lighter cakes, and repeat the process with the next heaviest cake
# (including the cakes just inserted). Stop when BOTH a substitution results in a 
# heist value <= the heavier cake AND you have checked all possible combinations of 
# lighter cakes for that substitution. Terminating the program after ONLY checking 
# for a substitution that results in a lower value may not be the best solution. See 
# test_skip_lower_value_cake in test_cake_thief.py file.

  else:
    duffel = sorted(duffel, key= lambda tup: tup[1], reverse=True)
    capacity += sum(y for x, y, z in duffel) # reset capacity to empty duffel. See next comments.
    for tup in duffel:
      ...
      cake_check(cakes_sorted_by_ratio, tup, duffel, capacity, remove_list)
    return final_view(duffel)
...
def cake_check(cakes_sorted_by_ratio, tup, duffel, capacity, remove_list):
  temp_heist = 0 # combined value of would-be replacements for the heavier cake
  better_cakes = [] # cakes that fit into the space left by the heavier cake
  for cake in cakes_sorted_by_ratio:
    ...
    # if a cake fits, add as many as possible, then try the next best cake.      
      ...
      while capacity >= cake[1]:
        better_cakes.append(cake) # list that will hold possible combination of replacement cakes
        capacity -= cake[1]
  ...
  # the combination of better_cakes and attempt to substitute again.      
