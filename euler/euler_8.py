...
def largest_product_in_series(series_length):
  ...
  for series in series_no_zeros:
    if len(series) < series_length:
      pass
    else:
      start, stop = 0, series_length
      while stop <= len(series):
        new_series = series[start:stop]
        for number in new_series:
          series_product *= int(number)
        if series_product > largest_product[0]:
          largest_product[0], largest_product[1] = series_product, new_series
        series_product = 1
